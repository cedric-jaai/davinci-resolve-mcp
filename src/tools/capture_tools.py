...
Desktop-wide capture tools (take_screenshot, capture_window_by_handle,
start/stop_screenshot_monitoring, list_all_windows) are gated behind the
ALLOW_DESKTOP_CAPTURE environment variable (default: false).
capture_resolve_ui (Resolve-window-only) is always available.
"""
...
def _desktop_capture_enabled() -> bool:
    """Check if desktop-wide capture is enabled via environment variable."""
    return os.environ.get("ALLOW_DESKTOP_CAPTURE", "false").lower() in (
        "true", "1", "yes",
    )
...
def register_capture_tools(mcp):
    ...
    # --- Always-available: Resolve-window-only capture ---
    ...
    # --- Always-available: informational resources (no image data) ---
    ...
    # --- Gated: desktop-wide capture tools ---
    # Only registered when ALLOW_DESKTOP_CAPTURE=true
    ...
    # Monitoring status is always available (informational)
